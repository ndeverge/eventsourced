...
case class Message(
  ...
  confirmationTarget: ActorRef = null,
  confirmationPrototype: Confirmation = null) {
 ...
 *
 * @param processorId Sending processor id.
 * @param channelId Sending channel id.
 * @param sequenceNr Message sequence number.
 * @param positive `true` for a positive confirmation, `false` otherwise.
 *
 */
case class Confirmation(processorId: Int, channelId: Int, sequenceNr: Long, positive: Boolean)

/**
 * Confirmation timeout message. System message, not used by applications.
 *
 * @param sequenceNr Sequence number of message for which a delivery confirmation timeout
 *        occurred.
 */
case class ConfirmationTimeout(sequenceNr: Long)
